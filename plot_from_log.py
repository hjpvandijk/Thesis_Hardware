...
TRAIL_LENGTH = 1  # Number of previous positions to keep for the trail
...
def cell_pheromone_from_string(node_string):
    """Parses a quadtree node string "x;y:pheromone"."""
    try:
        parts = node_string.split(":")
        x, y = map(float, parts[0].split(";"))
        pheromone = P(float(parts[1]))
        return x, y, pheromone
    except ValueError:
        return 0.0, 0.0, 0.0, 0.0
...
def draw_grid():
    ...
    #draw coordinates: (-1.695, 4.315), (-1.46, -3.73), (1.81, -3.72), (1.58, 4.315)
    coordinate1 = map_to_display_coordinates(-1.695, 4.315)
    coordinate2 = map_to_display_coordinates(-1.46, -3.73)
    coordinate3 = map_to_display_coordinates(1.81, -3.73)
    coordinate4 = map_to_display_coordinates(1.85, 4.315)
    coordinate5 = map_to_display_coordinates(0, 0)
    pygame.draw.circle(screen, (200, 0, 200), coordinate1, 5)
    pygame.draw.circle(screen, (200, 0, 200), coordinate2, 5)
    pygame.draw.circle(screen, (200, 0, 200), coordinate3, 5)
    pygame.draw.circle(screen, (200, 0, 200), coordinate4, 5)
    pygame.draw.circle(screen, (200, 0, 200), coordinate5, 5)
...
def get_first_and_last_timestamp():
    """Gets the first and last timestamp from the log file as datetime objects."""
    try:
        with open(LOG_FILE, "r") as log_file:
            ...
            if lines:
                first_line = lines[0]
                last_line = lines[-1]
                first_timestamp_str = " ".join(first_line.split(" - ")[0].split(" ")[:2])
                last_timestamp_str = " ".join(last_line.split(" - ")[0].split(" ")[:2])
                first_timestamp = datetime.strptime(first_timestamp_str, "%Y-%m-%d %H:%M:%S.%f")
...
def read_log_file():
    """Reads the log file and processes the messages."""
    try:
        with open(LOG_FILE, "r") as log_file:
            for line in log_file:
                ...
                if line:
                    # print(line)
                    on_message(None, None, line, True)
    except Exception as e:
        print(f"Error reading log file: {e}")


def read_log_file_until(timestamp):
    """Reads the log file and processes the messages."""
    try:
        with open(LOG_FILE, "r") as log_file:
            for line in log_file:
                line = line.strip()
                line_timestamp_str = " ".join(line.split(" - ")[0].split(" ")[:2])
                line_timestamp = datetime.strptime(line_timestamp_str, "%Y-%m-%d %H:%M:%S.%f")
...
def on_message(client, userdata, message, from_log):
    ...
    try:        
        ...
        if message_content.startswith("C:"):
        ...
        elif message_content.startswith("F:"):
            # if sender_id != MAP_AGENT_ID:
            #     return
            print("nothing")
            # try:
            ...
            #     for chunk in chunks:
            #         if chunk:
            #             x, y, pheromone = cell_pheromone_from_string(chunk)
            #             # agents[sender_id]['current_time'] = max(agents[sender_id]['current_time'], timestamp)
            #             # print(f"Timestamp: {timestamp}, Confidence: {pheromone}")
            #             # print(f"Current time: {agents[sender_id]['current_time']}")
            #             # x, y are cell coordinates.
            #             # if confidence > 0.5:
            #             #     map_data[(x, y)] = 1
            #             # elif confidence < -0.5:
            #             #     map_data[(x, y)] = 0
            #             add_to_data_map(x, y, pheromone, 0, 0, MAP_WIDTH_METERS, sender_id)
            # except Exception as e:
            #     #print stacktrace
            #     traceback.print_exc()
            #     print(f"Error parsing F message: {e}, content: {message_content}")
...
def main():
    ...
    try:
        ...
        i = 0
        # read_log_file()
        first_timestamp, last_timestamp = get_first_and_last_timestamp()
        # Initialize slider variables
        show_time = first_timestamp
        slider_width = 300
        slider_height = 20
        slider_x = (MAP_WIDTH_METERS * PIXELS_PER_METER - slider_width) // 2
        slider_y = MAP_HEIGHT_METERS * PIXELS_PER_METER - 50
        slider_rect = pygame.Rect(slider_x, slider_y, slider_width, slider_height)
        slider_knob_width = 10

        # Add a play button
        # play_button_rect = pygame.Rect(slider_x + slider_width + 20, slider_y, 50, slider_height)
        play_button_pos = map_to_display_coordinates(-4, 4)
        play_button_rect = pygame.Rect(play_button_pos[0], play_button_pos[1], 50, 50)
        is_playing = False

        def draw_play_button():
            pygame.draw.rect(screen, (0, 200, 0) if is_playing else (200, 0, 0), play_button_rect)
            play_text = font.render("▶" if not is_playing else "||", True, (255, 255, 255))
        ...
        def update_play_time():
            nonlocal show_time
            if is_playing:
                show_time += (last_timestamp - first_timestamp) / (slider_width * ticks_per_second)
            if show_time > last_timestamp:
                show_time = last_timestamp

        def draw_slider():
            pygame.draw.rect(screen, (200, 200, 200), slider_rect)  # Slider background
            knob_x = slider_x + int((show_time - first_timestamp).total_seconds() / (last_timestamp - first_timestamp).total_seconds() * slider_width)
            knob_rect = pygame.Rect(knob_x - slider_knob_width // 2, slider_y, slider_knob_width, slider_height)
            pygame.draw.rect(screen, (255, 0, 0), knob_rect)  # Slider knob

        def update_show_time(mouse_x):
            nonlocal show_time
            relative_x = mouse_x - slider_x
            relative_x = max(0, min(relative_x, slider_width))  # Clamp within slider bounds
            time_fraction = relative_x / slider_width
            show_time = first_timestamp + (last_timestamp - first_timestamp) * time_fraction
